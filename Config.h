#ifndef APPCONFIG_H
#define APPCONFIG_H
#include <QString>

const QString DEVICE1_NAME = "BabyMonitor1";
const QString DEVICE1_URL = "https://BabyMonitor1";
const QString DEVICE2_NAME = "BabyMonitor2";
const QString DEVICE2_URL = "https://BabyMonitor2";

const QString DEVICE1_CERT = QStringLiteral(R"(-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----)");

const QString DEVICE2_CERT = QStringLiteral(R"(asd)");

#endif // APPCONFIG_H
